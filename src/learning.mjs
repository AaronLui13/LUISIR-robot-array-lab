...
export const explanations = [
  'grid[0][2] 的 0 是行索引，2 是列索引；格內的 4 才是貨量。位置與內容要分開。',
  '(0,2) 是行 0、列 2；(2,0) 是行 2、列 0。交換兩個索引會指向另一格。',
  '寫入只改變指定格的內容；位置仍是 (1,2)，重新讀取會得到更新後的 5。',
  '外層固定目前行，內層走完該行各列。開始下一行時，內層列索引重新開始。',
  '逐行計數要在每行開始時歸零；每格歸零會丟失累計，只在最外層歸零則得到全表總數。',
  '搜尋值是條件；要找的是符合條件的行、列。本遊戲輸出位置後結束任務；筆記的 return 是把位置回傳給呼叫端，兩者不同。',
  '找全部位置時，輸出一個位置後要繼續遍歷；找第一個才在首次符合後停止。',
  ...
  '覆寫前先把原值存入 temp；第二次寫入才能取回原值，而不是再次複製已改變的格。temp 是暫存變量。',
  '逐列遍歷固定列 c，內層改變行 r。外層完成一列後才移到下一列。',
  '行索引 r 為奇數時（r 由 0 起），列索引由 cols−1 開始，每次減 1；終點填 −1（不包括），最後處理的列索引是 0。',
  '左右鏡像保留行，目的列是 cols−1−c；上下鏡像則保留列，反向行索引。',
  '順時針轉 90°：(r,c) → (c,rows−1−r)，新表的行列數互換。轉置則是 (r,c) → (c,r)。',
  '列索引由 1 開始，寬 3 格，即處理 1、2、3；終點不包括在內，所以填 4。',
...
export function hint(id, variant, level, mode) {
  if (level === 1) return id <= 2 ? '先指出位置，再讀格內數量。「?」要換成你的值或算式。' : '先逐步執行：第一個不符合預測的位置在哪裏？檢查該步的變量、來源格和輸出。';
