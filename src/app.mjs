...
function table(a,type='grid'){
 ...
 const visits=state().visited.filter(p=>equal(p,[r,c])).length,active=equal(type==='grid'?state().current:state().destination,[r,c]),chosen=type==='grid'&&selected.some(p=>equal(p,[r,c])),target=(id===1&&equal(mission().highlight,[r,c]))||(id===9&&equal([1,v===2?0:1],[r,c]))||(id===0&&phase==='predict'&&!locked&&equal([[0,2],[2,0]][selected.length],[r,c])),inRegion=id===15&&r<2&&c>=1;
...
function flowView(){
 ...
 const descriptions={predict:id===0&&!locked&&selected.length<2?'先找行，再找列。橙框就是這次要找的貨位。':'格內數字是貨量；索引是地址。預測不必一次答對。',build:mode==='arrange'?'用上下移動按鈕排好指令，再試行。':mode==='blank'?'從卡庫加入需要的指令，再試行。':'其他指令已準備好。只需填好橙色欄位，再按下方按鈕。',run:'每按一次，只做一小步。留意貨架和目前指令。',reflect:result?.ok?(quiz===mission().correct?'按下方綠色按鈕，繼續下一步。':'說清楚方法的意思，就可以繼續下一個任務。'):'按「返回修改指令」，保留你的想法繼續修正。'};
 return `<div class="flow-guide" tabindex="-1"><div><span>現在要做的事</span><h2>${titles[phase]}</h2><p>${descriptions[phase]}</p></div>${phase==='predict'&&locked?'<button class="text-button" data-action="back-build">繼續補指令 →</button>':phase==='build'?'<button class="text-button" data-action="back-predict">← 回看貨架／預測</button>':phase==='run'||phase==='reflect'?'<button class="text-button" data-action="back-build">← 返回修改指令</button>':''}</div>${flowError?`<p class="flow-error" role="alert">${escape(flowError)}</p>`:''}`;
}
