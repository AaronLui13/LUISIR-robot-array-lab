...
function viewBrief(){
 ...
 if(id===12&&v===1)return '完成蛇形遍歷後，再用算式輸出 (1,0) 的走訪編號；編號從 0 開始。';
 if(id===16&&v===1)return '找出所有 2×2 滑動視窗中總和最大的一個。起點步長為 1；輸出最大總和，再輸出左上角；同值保留最先遇到者。';
 if(id===16&&v>=2)return `這張貨架是 3×5，不能整除。已選邊界政策：${v===2?'捨去不能組成完整 2×2 區塊的底行及右列':'邊界以 0 補足後加總；只讀取原圖存在的格子。這不同於按實際格數求邊塊平均值'}。每個 2×2 區塊合成一格。`;
...
function render(){
 ...
 ${mappingView()}
 <div class="workbench">
 <section class="panel warehouse"><div class="panel-heading"><h2>${phase==='build'?'貨架參考圖':'倉庫貨架'} <small>${phase==='build'?'執行前':'Warehouse'}</small></h2><span class="dimensions">${grid().length} × ${grid()[0].length}</span></div><div class="warehouse-body">${robotConsole(s)}<div class="coordinate-note"><span>行索引 r ↓</span><span>列索引 c →</span></div>${table(phase==='build'?grid():s.grid)}<div class="legend"><span><i class="legend-square"></i> ${[13,14].includes(id)?'字母圖案':'每格數字 = 貨量'}</span><span><i class="legend-square dashed"></i> ${[13,14].includes(id)?'橙色 C = 追蹤點':id===9?'0 = 空格 · 橙框 = 起點':'0 = 空格'}</span></div><div class="selection-info">${s.current?`來源位置 <b>(${s.current.join(', ')})</b>${s.destination?` → 目的 <b>(${s.destination.join(', ')})</b>`:''}`:selected.length?`已選位置 <b>${selected.map(p=>'('+p.join(',')+')').join(' → ')}</b>`:'點選貨位，觀察行與列索引。'}</div>
 ${[7,13,14,16].includes(id)?`<div class="new-array"><h3>${id===7?'索引編號表':'結果陣列'} <span>result</span></h3>${id===7?'<p class="muted">0 是第一個合法編號，不是空位。此表練習位置換算，並未把元素存入一維陣列。</p>':''}${table(s.result,'result')}</div>`:''}
 ...
 <details class="python" data-details="code"><summary>展開遊戲指令／Python 對照 <span>含註解 · 同步追蹤</span></summary><label>程式顯示 <select id="code-format"><option value="pseudo" ${format==='pseudo'?'selected':''}>遊戲指令（0-based）</option><option value="python" ${format==='python'?'selected':''}>Python</option></select></label><pre class="code-view">${codeLines(cards,format).map(line=>`<span class="code-line ${line.index===s.card?'executing':''}">${escape(line.text)}</span>`).join('')}</pre><p>這是遊戲指令對照：索引由 0 起，結束值不包括。筆記的 1-based 偽代碼示範「由…至…」包括兩端，考試依題目定義。Python 片段須放在任務函數內；process 是遊戲自訂的走訪標記。print 輸出後繼續；return 離開函數，break 只離開最內層循環。結束任務卡不會回傳位置。</p></details></section>
