...
export const MISSIONS = [
 spec(0,0,'認識控制台','行 r 向下，列 c 向右；索引由 0 開始。先點選 (0,2)，再點選 (2,0)，讀取兩格。','依序輸出 4、0；完成兩個位置的點選。','預測兩次讀取的數量（用逗號分隔）','4,0','grid[0][2] 的 2 代表甚麼？',['貨物有 2 件','列索引是 2','第二行'],1),
 spec(1,1,'貨位與索引','先讀取 (0,2) 和 (2,0)。橙框標示另一個貨位：把它的行、列填進最後一張讀取卡。','依序輸出 4、0，以及橙框格的數量。','橙框的位置是？（行,列）','1,3','(0,2) 與 (2,0) 為甚麼不同？',['前者是行 0、列 2','兩個索引可以互換','索引代表貨量'],0,{highlight:[1,3]}),
 spec(2,1,'指定位置補貨','把 (1,2) 的貨量由 0 設定成 5。先在貨架點選你預計會改變的一格。','只有 (1,2) 變成 5，其餘保持原值。','重新讀取 grid[1][2]，預計得到甚麼？','5','同一位置為甚麼讀到不同數字？',['位置改變了','索引從 1 開始','該格儲存的值已被更新'],2),
 spec(3,2,'遍歷所有貨位','把「每行」「每列」「檢查目前格」組成嵌套循環。縮排表示指令屬於哪個循環。','12 格全部處理，每格恰好一次。','逐行遍歷的第二個位置是？（行,列）','0,1','內層循環負責甚麼？',['走完目前行的所有列','只檢查第一格','改變貨物數量'],0),
 spec(4,2,'空位報告','每格數字是貨量；0 代表空位。遍歷時符合條件才把計數器加 1。','輸出空格總數；變式要逐行輸出。','共用貨架有幾個空位？','4','每行分別計數，count 應在哪裏歸零？',['最內層每格之前','每一行開始時','所有行完成後'],1,{variants:['整個貨架','每行空位']}),
 ...
 spec(6,3,'第一個？還是全部？','搜尋貨量 2。比較輸出第一個與輸出所有位置，調整停止的時機。','第一個 (0,0)；全部 (0,0)、(1,3)、(2,1)。','共有幾格的貨量是 2？','3','要找齊所有位置，找到一格後應怎樣？',['立刻結束本次任務','繼續搜尋餘下格子','把搜尋值加 1'],1,{variants:['第一個','全部位置','搜尋不存在的 9']}),
 ...
 spec(8,4,'只剩一張編號單','編號單只寫了 9。利用整數除法 DIV（//）及取餘數 MOD（%）還原行列索引。','每行 4 格 → (2,1)；每行 5 格 → (1,4)。','每行 4 格時，編號 9 的位置？（行,列）','2,1','為甚麼除以 cols？',['每一完整行有 cols 格','因為總共有 cols 行','任何常數都可以'],0,{variants:['每行 4 格','每行 5 格']}),
 ...
 spec(10,5,'交換兩個貨位','先觀察「直接覆寫」的錯誤。用暫存變量 temp 交換 (0,0) 與 (1,2)。','2 和 5 成功交換，沒有遺失或複製貨量。','交換後 grid[0][0] 是多少？','5','temp 的用途是甚麼？',['保存即將被覆寫的原值','改變索引','讓貨量加倍'],0),
 spec(11,6,'改變遍歷方向','把逐行改成逐列：外層控制列，內層控制行。貨架內容不可改變。','按 (0,0)、(1,0)、(2,0)、(0,1)… 走訪。','前四個位置？例如 0,0;1,0;2,0;0,1','0,0;1,0;2,0;0,1','逐列遍歷的外層循環控制甚麼？',['行 r','列 c','貨量'],1),
 spec(12,6,'蛇形遍歷','行索引 r 由 0 起：r 為偶數時向右，為奇數時向左。加入 r % 2 判斷及倒序範圍。','每格一次；第二行由 (1,3) 開始。','(1,0) 的蛇形走訪編號是？','7','行索引 r 為奇數時，列索引的範圍應是？',['0 到 cols-1，步長 1','cols-1 到 0，步長 -1','cols 到 1，步長 -1'],1,{variants:['蛇形遍歷','蛇形編號挑戰']}),
 ...
 spec(15,8,'只統計這一區','區域左上角 (0,1)、高 2、寬 3。只處理框內六格並累加。','六格各一次，輸出總和 10；框外不參與。','這六格的總和是？','10','循環終點不包括在內，列的終點應填？',['3','4','2'],1,{region:[0,1,2,3]}),
 spec(16,8,'縮小倉庫地圖','每個不重疊 2×2 區塊合成一格。從結果索引計算來源起點。','建立 2×2 結果；每塊獨立累加，每來源格恰好一次。','左上 2×2 區塊的總和是？','6','不重疊區塊的來源起點步長是多少？',['1','2','4'],1,{variants:['2×2 合併','滑動視窗最大總和','只保留完整區塊','補零後加總']}),
...
export function validate(id,v,grid,state,cards) {
  ...
  if(id<=1&&!equal(state.reads,id===0?[[0,2],[2,0]]:[[0,2],[2,0],[1,3]]))return {ok:false,message:'請實際讀取指定位置，而不是直接輸出預計答案。'};
  ...
  if(e.visited){
    ...
    if(!equal(order?actual:actual.sort(),order?wanted:wanted.sort()))return {ok:false,message:actual.length>wanted.length?'有格子被重複處理。檢查循環範圍。':'走訪的格子或次序不正確。檢查外層與內層循環。'};
  }
  if(e.first&&e.found.length&&!state.stopped)return {ok:false,message:'已找到位置，但程式沒有停止；加入「結束本次任務」。'};
  if([5,6].includes(id)) {
  ...
  if(id>=3&&![8,9,10].includes(id)&&!cards.some(c=>c.op==='for'))return {ok:false,message:'請使用循環指令建立可以重用的方法。'};
...
export const CARD_TYPES = {
 ...
 write:{label:'修改元素值',english:'Write',stage:0,args:{r:'0',c:'0',value:'5'}},
 output:{label:'輸出數值',english:'Output',stage:0,args:{value:'value'}},
 for:{label:'循環',english:'For loop',stage:2,args:{name:'r',from:'0',to:'rows',step:'1'}},
 inspect:{label:'檢查目前格',english:'Visit',stage:2,args:{r:'r',c:'c'}},
 set:{label:'變量賦值',english:'Assignment',stage:2,args:{name:'total',value:'0'}},
 if:{label:'如果',english:'If',stage:2,args:{test:'grid[r][c] == 0'}},
 record:{label:'輸出位置',english:'Position',stage:3,args:{r:'r',c:'c'}},
 stop:{label:'結束本次任務',english:'Stop',stage:3,args:{}},
 new:{label:'建立結果陣列',english:'New array',stage:4,args:{rows:'rows',cols:'cols'}},
 copy:{label:'寫入結果陣列',english:'Write',stage:4,args:{r:'r',c:'c',value:'r*cols+c'}}
};
export const FIELD_LABELS={r:'行索引 r',c:'列索引 c',value:'值／算式',name:'變量',from:'由（包括）',to:'至（不包括）',step:'步長',test:'條件',rows:'新表行數',cols:'新表列數'};
